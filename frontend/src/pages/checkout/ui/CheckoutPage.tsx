...
import { useCheckout, useConfirmPayment } from '@/entities/payment'
...
export function CheckoutPage() {
  ...
  const pay = async () => {
    ...
    setError(null)
    try {
      const payment = await checkout.mutateAsync({ courseId, promoCode: promo || undefined })
  ...
  return (
    <div className="container max-w-lg py-12">
      ...
      <div className="mt-8 border border-line bg-surface p-6">
        ...
        <Button onClick={pay} size="lg" disabled={busy} className="mt-5 w-full">{busy ? t('checkout.paying') : t('checkout.pay')}</Button>
