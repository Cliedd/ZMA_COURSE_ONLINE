...
describe('CurriculumEditor', () => {
  it('met à jour la section et inclut mediaId dans le programme quand une vidéo est téléversée', async () => {
    mswServer.use(
      http.post(`${API}/media/presign`, () =>
        HttpResponse.json(
          { mediaId: 'media-42', uploadUrl: '/api/v1/media/media-42/upload-direct', s3Key: 'local/media-42/lesson.mp4', expiresInSeconds: 3600 },
          { status: 201 },
        ),
      ),
      http.post(`${API}/media/media-42/upload-direct`, () =>
        HttpResponse.json({ id: 'media-42', fileName: 'lesson.mp4', size: 1024, contentType: 'video/mp4', status: 'READY' }),
      ),
    )
